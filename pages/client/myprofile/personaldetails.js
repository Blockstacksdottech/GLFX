...
import { patchReq, postReq, req } from "@/helpers/helpers";
...
export default function Personaldetails() {
  ...
  const fetchInfo = async () => {
    const resp = await req("personalinfo");
    if (resp) {
      setInfo(resp);
      setLoading(false);
    } else {
  ...
  const updateUserInfo = async () => {
    const resp = await postReq("userinfo", userInfo);
    if (resp) {
      let temp = { ...User };
  ...
  return (
    <>
      <Head>
        <title>GLFX - My Profile | Personal Details</title>
      </Head>
      <Checker admin={false}>
        {!loading && info && (
          <>
                ...
                <main className="col-md-9 ms-sm-auto col-lg-10 px-md-4 bg-grey">
                  <div className="pt-3 pb-2 mb-3 border-bottom">
                    <div className="clearfix">
                      <h1 className="h5 text-center">Personal Details</h1>
                    </div>
                  </div>
